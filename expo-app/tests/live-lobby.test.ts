...
describe("one-to-one live lobby models", () => {
  ...
  it("decodes real/legacy page aliases, host user, billing, gender and call capabilities", () => {
    const page = normalizeLiveSlotPage({
      slots: [{
        ...
        host: { user_id: 9, username: "alice", nickname: "Alice", avatar_url: "/a.jpg", gender: "female" },
      }],
      next_cursor: "next",
      billing_policy: { free_seconds: "12", unit_seconds: 60, amount_per_unit: "120", minimum_starting_balance: 240 },
    ...
    expect(page).toMatchObject({
      ...
      billingPolicy: { freeSeconds: 12, unitSeconds: 60, amountPerUnit: 120, minimumStartingBalance: 240 },
      items: [{ id: "81", allowedCallTypes: ["voice", "video"], user: { userId: "9", nickname: "Alice" } }],
    });
  ...
  it("maps every native availability alias and hides ended slots", () => {
    expect(["waiting", "available", "idle"].map(liveAvailability)).toEqual(["available", "available", "available"]);
    expect(["pending", "mystery", "connecting", "closed"].map(liveAvailability)).toEqual(["inviting", "unknown", "busy", "ended"]);
  ...
  it("sorts available first while retaining server order inside each rank", () => {
    const sorted = sortLiveSlots([slot("busy-1", "in_call"), slot("free-1", "waiting"), slot("unknown", "new"), slot("free-2", "idle")]);
  ...
  it("normalizes call-state phases and stable call type order", () => {
    expect(normalizeLiveCallState({ call_id: "c", slot_id: "s", status: "in-call", call_type: "audio" }))
      .toMatchObject({ callId: "c", slotId: "s", status: "in_call", phase: "accepted", callType: "voice" });
    expect(normalizeLiveCallState({ call_id: "c", status: "expired" }).phase).toBe("terminal");
    expect(normalizeLiveCallState({ call_id: "c", status: "ended", end_reason: "billing_insufficient", termination_grace_ms: "3200", final_billing: { charged_activity_cat_food: "20", charged_gold_coins: 80, total_charged: 100, spendable_balance_after: 4, billing_status: "billing_insufficient" } })).toMatchObject({
      endReason: "billing_insufficient",
      terminationGraceMilliseconds: 3200,
      finalBilling: { chargedActivityCatFood: 20, chargedGoldCoins: 80, totalCharged: 100, spendableBalanceAfter: 4, billingStatus: "billing_insufficient" },
    });
  ...
  it("normalizes the server-confirmed reserved experience on an invitation", () => {
    expect(normalizeLiveCallInvitation({
  ...
  it("sanitizes every unsafe billing-policy field like the native initializer", () => {
    expect(normalizeLiveBillingPolicy({
      currency: "",
      free_seconds: -1,
      unit_seconds: 0,
      amount_per_unit: -8,
  ...
  it("normalizes root-level live slot event aliases used by the native cursor", () => {
    expect(normalizeLiveLobbySlotEvent({
      event_id: 8,
      slot_id: "slot-8",
      status: "CLOSED",
      host_id: "host-8",
  ...
  it("remembers stale event IDs so a replay cannot mutate state later", () => {
    ...
    expect(cursor.shouldApply({ slot: null, eventId: "new", slotId: "slot", occurredAt: 200 })).toBe(true);
    expect(cursor.shouldApply({ slot: null, eventId: "new", slotId: "slot", occurredAt: 200 })).toBe(false);
    expect(cursor.shouldApply({ slot: null, eventId: "stale", slotId: "slot", occurredAt: 100 })).toBe(false);
    expect(cursor.shouldApply({ slot: null, eventId: "stale", slotId: "another" })).toBe(false);
    expect(cursor.shouldApply({ slot: null, eventId: "newer", slotId: "slot", occurredAt: 300 })).toBe(true);
  });
  ...
  it("matches native current-slot fallback for unsupported and transient endpoints", () => {
    ...
    expect(reconcileCurrentLiveSlot({ kind: "value", slot: slot("ended", "ended", "me") }, previous, [own], "me")).toBeNull();
  });
