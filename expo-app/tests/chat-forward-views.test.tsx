...
jest.mock("@/providers/LocalizationProvider", () => ({ useLocalization: () => ({ t: mockTranslate }) }));
...
jest.mock("@/components/GroupMemberAvatar", () => {
  const { Text: MockText } = jest.requireActual("react-native");
  return { GroupMemberAvatar: ({ groupId }: { groupId: number }) => <MockText>{`group-avatar:${groupId}`}</MockText> };
});
...
describe("batch selection and forwarding UI", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    friends.mockResolvedValue([{ user_id: "u1", nickname: "Alice", avatar_url: "/a.jpg", added_at: "2026-08-07T00:00:00Z" }]);
  ...
  it("exposes the original selection toolbar actions and 230pt bundle card tap", async () => {
    ...
    await act(async () => { fireEvent.press(view.getByText("转发")); });
    await act(async () => { fireEvent.press(view.getByText("删除")); });
    await act(async () => { fireEvent.press(view.getByLabelText("聊天记录，3")); });
  ...
  it("loads direct/group targets, multi-selects, confirms and sends one operation", async () => {
    ...
    const sources = [{ conversation_type: "dm" as const, conversation_id: "source", message_id: 7, expected_version: 1 }];
    const view = await render(
      <ForwardFlowModal mode="individual" onClose={onClose} onCompleted={onCompleted} preview="两条消息" sources={sources} visible />,
    );
    ...
    await act(async () => { fireEvent.press(view.getByText("多选")); });
    await act(async () => { fireEvent.press(view.getByText("Study Group")); });
    await act(async () => { fireEvent.press(view.getByText("Alice")); });
    await act(async () => { fireEvent.press(view.getByText("完成")); });
    expect(view.getByText("Alice、Study Group")).toBeTruthy();
    expect(view.getByText("两条消息")).toBeTruthy();
    await act(async () => { fireEvent.press(view.getByText("发送")); });
    await waitFor(() => expect(submitForward).toHaveBeenCalledWith({
  ...
  it("shows the load failure state and retries both target sources", async () => {
    ...
    const view = await render(
      <ForwardFlowModal mode="single" onClose={jest.fn()} onCompleted={jest.fn()} preview="hello" sources={[]} visible />,
    );
    await waitFor(() => expect(view.getByText("加载失败")).toBeTruthy());
    expect(view.getByText("offline")).toBeTruthy();
    await act(async () => { fireEvent.press(view.getByText("重试")); });
    await waitFor(() => expect(view.getByText("Alice")).toBeTruthy());
  ...
  it("loads merged bundle detail, localizes voice rows and closes", async () => {
    loadBundle.mockResolvedValue({
      ...
      items: [
        { ordinal: 1, sender_name: "Alice", sent_at: "2026-08-07T10:00:00Z", message_type: "text", summary: "hello" },
        { ordinal: 2, sender_name: "Bob", sent_at: "2026-08-07T10:01:00Z", message_type: "voice", summary: "2s" },
      ],
    });
    const onClose = jest.fn();
    ...
    await act(async () => { fireEvent.press(view.getByText("关闭")); });
