...
describe("native chat-money contracts", () => {
  ...
  it("keeps the exact bubble, composer, envelope and color metrics", () => {
    ...
    expect(chatMoneyComposerPolicy).toMatchObject({
      inputRowHeight: 64,
    ...
    expect(chatMoneyDetailPolicy).toMatchObject({
      overlayOpacity: 0.52,
    ...
    expect(chatMoneyTheme).toMatchObject({
      cardOrange: "#FA9D3B",
  ...
  it("parses aliases strictly and never exposes red-packet amount in encoded message content", () => {
    const payload = parseChatMoneyPayload(
      JSON.stringify({
        assetId: "red-2",
        kind: "redPacket",
        scope: "private",
    ...
    expect(JSON.parse(encodeChatMoneyPayload({ ...payload!, amount: 999 }))).not.toHaveProperty(
      "amount",
    );
    expect(parseChatMoneyPayload(JSON.stringify({ data: { asset_id: "nested" } }))).toBeNull();
    const transfer = parseChatMoneyPayload(
      JSON.stringify({
        asset_id: "tr-1",
        kind: "transfer",
        scope: "direct",
  ...
  it("normalizes fail-closed configuration, flexible detail fields and terminal permissions", () => {
    expect(
      ...
      limits: {
        minimum_amount: 2,
        maximum_amount: 1000,
        maximum_packet_count: 20,
        red_packet_minimum_amount: 2,
        transfer_maximum_amount: 1000,
      },
    ...
    expect(detail).toMatchObject({
      ...
      version: 4,
      can_claim: true,
      claims: [{ user_id: "u1", nickname: "One", amount: 8, is_luckiest: true }],
    });
  });
    ...
    const equal = validateChatMoneyComposer(
      {
        ...
        limits: {
          ...
          red_packet_maximum_amount: 20000,
          transfer_minimum_amount: 1,
          transfer_maximum_amount: 20000,
  ...
  it("enforces sender/group/local-claim/enforced-server envelope and open-action rules", () => {
    ...
    expect(
      shouldShowRedPacketEnvelopeFromPayload(
        redPayload({ scope: "dm", mode: "direct" }),
        true,
        false,
      ),
    ).toBe(false);
  ...
  it("parses nested receipt envelopes to depth four and localizes by viewer role", () => {
    const receipt = normalizeChatMoneyReceipt(
      JSON.stringify({
        data: {
          receipt_message: {
            payload: {
              assetId: "red-1",
              eventType: "red_packet_claimed",
              actorId: "me",
              actorName: "Me",
              senderId: "sender",
              senderName: "Sender",
    ...
    expect(receipt).toMatchObject({
      event_id: "red-1:red_packet_claimed:me",
      asset_id: "red-1",
      event_type: "red_packet_claimed",
      actor_id: "me",
    });
    expect(localizedChatMoneyReceipt(receipt!, "me", t)).toBe(
      "chatMoney.receipt.claimedByMe:Sender",
    );
  });
  ...
  it("merges red-packet claim history append-only and blocks terminal permissions", () => {
    const merged = mergeChatMoneyDetail(
      redDetail({
        claims: [
          { user_id: "one", nickname: "One", amount: 3, claimed_at: "a", is_luckiest: false },
        ],
        version: 2,
      }),
      redDetail({
        claims: [
          { user_id: "two", nickname: "Two", amount: 4, claimed_at: "b", is_luckiest: true },
        ],
        status: "completed",
        version: 3,
        can_claim: true,
  ...
  it("posts exact red-packet and transfer creation bodies and normalizes messages", async () => {
    request
      .mockResolvedValueOnce({
        message: {
          ...
          receiver_id: "friend",
          msg_type: "red_packet",
          content: encodeChatMoneyPayload(redPayload({ scope: "dm", mode: "direct" })),
        },
    ...
    const red = await createRedPacketMessage({
      clientMessageId: "red-key",
      scope: "dm",
      mode: "direct",
      totalAmount: 10,
      packetCount: 1,
    ...
    expect(request).toHaveBeenNthCalledWith(1, "/wallet/red-packets", {
      method: "POST",
      headers: { "Idempotency-Key": "red-key" },
      body: {
        client_message_id: "red-key",
        scope: "dm",
        mode: "direct",
        total_amount: 10,
        packet_count: 1,
    ...
    expect(request).toHaveBeenNthCalledWith(2, "/wallet/transfers", {
      method: "POST",
      headers: { "Idempotency-Key": "tr-key" },
      body: {
        client_message_id: "tr-key",
        scope: "group",
        recipient_id: "friend",
        amount: 88,
        note: "Dinner",
  ...
  it("uses exact configuration/detail/claim/accept/return endpoints", async () => {
    request
      ...
      .mockResolvedValueOnce({
        detail: redDetail({ status: "completed", version: 2 }),
        payload: redPayload({ status: "completed", version: 2 }),
      })
      .mockResolvedValueOnce({
        detail: { ...redDetail(), asset_id: "tr", kind: "transfer", status: "accepted", amount: 9 },
        payload: {
          asset_id: "tr",
          kind: "transfer",
          scope: "dm",
      ...
      .mockResolvedValueOnce({
        detail: { ...redDetail(), asset_id: "tr", kind: "transfer", status: "returned", amount: 9 },
        payload: {
          asset_id: "tr",
          kind: "transfer",
          scope: "dm",
  ...
  it("restores detail and configuration caches after memory is recreated", async () => {
    ...
    await expect(loadChatMoneyConfiguration("owner-a")).resolves.toMatchObject({
      red_packet_enabled: true,
    });
    await expect(
      loadChatMoneyDetail({ ownerId: "owner-a", assetId: "persisted-asset" }),
    ).resolves.toMatchObject({ asset_id: "persisted-asset" });
