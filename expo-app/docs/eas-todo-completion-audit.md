...
| 验收标准                    |   21 | ⚠️   | Expo/RN/TS、双端构建能力、Router、Build/Submit/Update 配置、fingerprint、三环境、channel 隔离、Preview OTA、Production 灰度、rollback、非阻断、Remote Config/flags/cache、文档、secret scan 与本地质量门均满足；Android Production AAB 与 iOS Distribution-signed Ad Hoc IPA 均通过。未闭环的是 iOS EAS Store/TestFlight 云构建和真实 Sentry 接收。 |
...
1. EAS credentials 当前仍没有 iOS Store 凭据。已登记在当前 Ad Hoc Profile 的 75 台内部设备可以先使用本机已签 IPA，不经蒲公英安装并接收 Production OTA；未登记设备或 TestFlight/App Store 仍需有效 Apple Developer 账号在交互式流程自动配置后重跑 `pnpm build:prod:ios`。也可由用户自行安全导出指定证书私钥后手工上传；Codex 没有擅自导出私钥。
...
以上外部项不会改变 47/47 页面复刻与已经证明的 EAS Update/灰度/撤销/rollback 能力。当前可以声称“Ad Hoc Profile 已登记的内部 iPhone 已有可安装 Production 包”；不能声称未登记设备可安装，也不能把它描述成已完成的 EAS Store/TestFlight Build。
