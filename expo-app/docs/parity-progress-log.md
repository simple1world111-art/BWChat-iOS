...
- **Production Build**：Android Store Build `4f2e2e67-2ab9-43e7-b87f-4c458a72c24d` 已 `FINISHED`，runtime/commit 匹配；139,077,166-byte AAB 的 SHA-256 为 `80999fe7e2f08b3276e8f0d4e8cfb4f0f7099a931bdd51c7d5445c3d5188169f`，ZIP 无错误并包含四种 ABI 下的 App Modules/Expo Updates，临时下载已清理。iOS Production Store Build 已通过代码/runtime 阶段，但 EAS 因尚无 Apple Distribution Certificate / Provisioning Profile 拒绝非交互签名；凭据页权威显示 `No credentials set up yet!`，这是 Apple Developer 外部门槛，不是代码或 Expo OTA 失败。
