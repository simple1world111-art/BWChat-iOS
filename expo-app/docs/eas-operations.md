...
2. 再次冷启动，确认新 Update ID 已静默生效。
3. 核对登录、消息、地图、发现、我的与本次改动页面。
4. 断网冷启动，确认仍能进入上次可用版本。

客户端冷启动会立即检查一次；从后台回到前台时按 15 分钟间隔检查。Channel 上积累多次兼容更新时，客户端只静默下载最新完整 bundle，不逐个安装历史更新。运行中的 JavaScript 无法在不重启 App 的情况下替换，因此下载完成后不弹窗、不强制重启，而是在用户下一次自然冷启动时自动生效。
