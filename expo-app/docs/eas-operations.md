...
2. 下载完成后确认只出现一次累积更新提示；点击“立即重启”后确认新 Update ID 生效。
3. 核对登录、消息、地图、发现、我的与本次改动页面。
4. 断网冷启动，确认仍能进入上次可用版本。

客户端冷启动会立即检查一次；从后台回到前台时按 15 分钟间隔检查。Channel 上积累多次兼容更新时，客户端只下载最新完整 bundle，并按最新 Update ID 提示一次，不逐个安装历史更新。首次发布“自动弹窗”能力时，尚未包含该能力的旧客户端会先静默下载，并需自然冷启动一次；从下一次 OTA 起即可在下载完成后直接弹出“稍后 / 立即重启”，无需进入“更新与诊断”。
...
iOS Production Store 云构建也已闭环：EAS Build `2236972b-d74c-417f-9f52-18e48cd8e8e3` 为 `FINISHED`，最终 `1.0.0 (11)` IPA 为 49,697,034 bytes、SHA-256 `92c5d2eca5826555cd88fc145e91bfe04461339180a81c670e7a64e5b5ed8016`。ZIP、arm64、App Store Profile、Distribution 深度签名、Production APNs/communication entitlement、无 Dev Launcher/Menu、`production` channel 与 runtime `610f9a3e005a9939903c424963e89631d7be538f` 全部通过。ASC App ID 为 `6760574012`；Submission `705eafd8-8d74-4531-9926-ae90e21b9e63` 已成功上传 App Store Connect。Apple 已处理完成，Build 11 为 `Ready to Submit`、90 天有效；内部组与单独测试人员均为 0。外部 `BWChatAdmin` 组已有公开链接 `https://testflight.apple.com/join/1hmTBWBA`，但当前 `0 Builds / 0 Testers`。`ascAppId` 没有固化到当前 `eas.json`：实测该非原生提交字段仍会改变当前 fingerprint，撤回后才能保持已安装包/稳定 OTA 的 `610f9a3e...` runtime。临时 IPA/解包目录已删除，只保留 `artifacts/verification/eas-production-ios-store/verification.md`。

仍未完成并且不得提前声称完成：

- Apple 已处理 iOS Build 11，但外部 Beta Review 表单还需要用户指定 Feedback Email，并明确授权把测试账号交给 Apple；完成后还要等待 Beta Review。当前不声称公开链接已经可以安装。
- Android Production Store Build `4f2e2e67-2ab9-43e7-b87f-4c458a72c24d` 已 `FINISHED`；AAB 为 139,077,166 bytes、SHA-256 `80999fe7e2f08b3276e8f0d4e8cfb4f0f7099a931bdd51c7d5445c3d5188169f`，ZIP 与四种 ABI 的 Expo Updates/App Modules 原生库已核对，临时下载已删除。Android Production Submit 仍需 Google Play service account/track。
- 可选 Sentry project/DSN/source map 上传尚未验收，不阻断当前 EAS Build/Update/TestFlight 上传结论。
- GitHub `preview`/`production` Environments、Production Required reviewers、实际发布/回滚人员名单与 `EXPO_TOKEN` Secret 尚未由仓库管理员配置。
- Preview 双端构建、iOS Preview OTA、Production OTA/灰度/撤销/rollback、双端 Production Store Build 与 iOS App Store Connect 上传/处理已完成；已登记的内部 iPhone 可立即用本机 Ad Hoc IPA 实现“一次安装后持续收到 Production OTA”，未登记设备等待 Beta Review 信息、审核和外部组启用。Android 可先使用已完成的 Preview APK 做内部安装与同 runtime 的 Preview OTA。
