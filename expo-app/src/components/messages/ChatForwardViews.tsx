...
export function ChatSelectionIndicator({ selected }: { selected: boolean }) {
  ...
  return (
    <View
      accessibilityLabel={t(selected ? "selection.selected" : "selection.notSelected")}
      style={styles.selectionHitArea}
    >
      <SymbolView
...
export function ForwardBundleMessageCard({
  ...
  return (
    <Pressable
      accessibilityLabel={`${payload.title}，${payload.item_count}`}
      ...
      style={[styles.bundleCard, isFromMe && styles.mineBundleCard]}
    >
      <View style={styles.bundleTitleRow}>
        <SymbolView name="bubble.left.and.bubble.right.fill" size={18} tintColor={colors.accent} />
        <Text numberOfLines={1} style={styles.bundleTitle}>
          {payload.title}
        </Text>
      </View>
      <Text numberOfLines={3} style={styles.bundleSummary}>
        {payload.summary}
      </Text>
...
export function ForwardFlowModal({
  ...
  const filtered = useMemo(() => {
    ...
    return normalized
      ? targets.filter((target) => target.display_name.toLocaleLowerCase().includes(normalized))
      : targets;
  }, [query, targets]);
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      ...
      setError(null);
      setLoading(true);
      void Promise.allSettled([getFriendList(), getGroups()])
        .then(([friendsResult, groupsResult]) => {
          if (cancelled) return;
          const nextTargets = [
            ...(friendsResult.status === "fulfilled" ? friendsResult.value : []).map(
              (friend): ForwardTarget => ({
                conversation_type: "dm",
                conversation_id: friend.user_id,
                display_name: friend.nickname,
                avatar_url: friend.avatar_url,
              }),
            ),
            ...(groupsResult.status === "fulfilled" ? groupsResult.value : []).map(
              (group): ForwardTarget => ({
                conversation_type: "group",
                conversation_id: String(group.group_id),
          ...
          const failure =
            friendsResult.status === "rejected"
              ...
              : groupsResult.status === "rejected"
                ...
                : null;
          const message =
            failure instanceof Error ? failure.message : failure ? t("common.loadFailed") : null;
  ...
  const submit = async () => {
    ...
    try {
      ...
      Alert.alert(
        t("common.error"),
        nextError instanceof Error ? nextError.message : t("messages.sendFailed"),
      );
    } finally {
      setSending(false);
    }
      ...
      <ForwardModalSafeArea>
        <View style={styles.forwardHeader}>
          ...
          {isMultiTarget ? (
            <Pressable
              ...
              onPress={() => setConfirmationTargets(sortForwardTargets(selected))}
            >
              <Text style={[styles.headerAction, selected.length === 0 && styles.disabledText]}>
                {t("common.done")}
              </Text>
            </Pressable>
          ) : (
            <Pressable onPress={() => setMultiTarget(true)}>
              <Text style={styles.headerAction}>{t("chat.action.multiSelect")}</Text>
            </Pressable>
          )}
        </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const checked = selected.some(
              (target) => forwardTargetKey(target) === forwardTargetKey(item),
            );
            const numericGroupId =
              item.conversation_type === "group" ? Number(item.conversation_id) : Number.NaN;
            return (
              <Pressable onPress={() => selectTarget(item)} style={styles.targetRow}>
                ...
                {isMultiTarget ? (
                  <SymbolView
                    ...
                    size={22}
                    tintColor={checked ? colors.accent : colors.tertiaryText}
                  />
                ) : null}
              </Pressable>
            );
        ...
        {confirmationTargets.length > 0 ? (
          <View style={styles.confirmationBackdrop}>
            <Pressable
              disabled={sending}
              onPress={() => setConfirmationTargets([])}
              style={StyleSheet.absoluteFill}
            />
            <View style={styles.confirmationCard}>
              <View style={styles.confirmationHandle} />
              <Text numberOfLines={2} style={styles.confirmationTitle}>
                {confirmationTargets.map((target) => target.display_name).join("、")}
              </Text>
              <Text numberOfLines={3} style={styles.confirmationPreview}>
                {preview}
              </Text>
...
export function ForwardBundleDetailModal({
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      setBundle(null);
      setError(null);
      void getForwardBundle(bundleId)
        .then((nextBundle) => {
      ...
      <ForwardModalSafeArea>
        <View style={styles.forwardHeader}>
          ...
          <Text numberOfLines={1} style={styles.headerTitle}>
            {bundle?.title ?? t("forward.chooseChat")}
          </Text>
            ...
            {bundle.items.map((item) => (
              <View key={item.ordinal} style={styles.bundleDetailRow}>
                <View style={styles.bundleDetailHeader}>
                  <Text style={styles.bundleDetailSender}>{item.sender_name}</Text>
                  <Text style={styles.bundleDetailTime}>{formatForwardTime(item.sent_at)}</Text>
                </View>
                <Text style={styles.bundleDetailSummary}>
                  {item.message_type === "voice" ? t("message.voice") : item.summary}
                </Text>
              </View>
            ))}
          </ScrollView>
...
function ForwardModalSafeArea({ children }: PropsWithChildren) {
  return (
    <SafeAreaProvider initialMetrics={initialWindowMetrics}>
      <SafeAreaView
        edges={["top", "bottom"]}
        style={styles.forwardScreen}
        testID="forward.safeAreaScreen"
      >
        {children}
      </SafeAreaView>
    </SafeAreaProvider>
...
function SelectionAction({
  ...
  title: string;
  icon: "arrowshape.turn.up.right" | "trash";
  disabled: boolean;
  destructive?: boolean;
  onPress: () => void;
}) {
  return (
    <Pressable
      disabled={disabled}
      onPress={onPress}
      style={[styles.selectionAction, disabled && styles.disabledAction]}
    >
      <SymbolView name={icon} size={20} tintColor={destructive ? colors.danger : colors.accent} />
      <Text style={[styles.selectionActionText, destructive && styles.destructiveText]}>
        {title}
      </Text>
    </Pressable>
  );
