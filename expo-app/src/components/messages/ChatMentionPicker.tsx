...
export function ChatMentionPicker({
  ...
  const filtered = useMemo(() => {
    ...
    return normalized
      ? members.filter(
          (member) =>
            member.nickname.toLocaleLowerCase().includes(normalized) ||
            member.user_id.toLocaleLowerCase().includes(normalized),
        )
      : members;
  }, [members, query]);
  ...
  const finishMultiSelection = () => {
    ...
    if (selectedIds.has("mention:all"))
      selections.push({ kind: "all", nickname: t("mention.all") });
    for (const member of members) {
      const selection: ChatMentionSelection = {
        kind: "direct",
  ...
  return (
    <Modal
      ...
      testID="mention.fullScreenModal"
      visible
    >
      <SafeAreaProvider initialMetrics={initialWindowMetrics}>
        ...
        >
            ...
            >
              <Text
                style={[
                  ...
                  isMultiSelecting && selectedIds.size === 0 && styles.disabled,
                ]}
              >
                {t(isMultiSelecting ? "common.done" : "mention.multiSelect")}
              </Text>
            </Pressable>
          </View>
          ...
          {!isLoading && !error && rows.length === 0 && query.trim() ? (
            <View style={styles.center}>
              <Text style={styles.errorText}>{t("mention.noResults")}</Text>
            </View>
          ) : null}
          {members.length > 0 || rows.length > 0 ? (
            <FlatList
              data={rows}
              keyExtractor={(item) =>
                item.kind === "all" ? "mention:all" : `mention:${item.member.user_id}`
              }
              refreshControl={
                <RefreshControl
    ...
    <Pressable onPress={onPress} style={styles.row}>
      {userId ? (
        ...
        <View style={styles.allAvatar}>
          <SymbolView name="person.3.fill" size={16} tintColor={colors.accent} weight="semibold" />
        </View>
      )}
